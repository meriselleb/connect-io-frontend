import React, { Component, Fragment } from 'react';
import PropTypes from 'prop-types';
import withStyles from '@material-ui/core/styles/withStyles';
import { Link } from 'react-router-dom';
import dayjs from 'dayjs';
// Material UI
import Grid from '@material-ui/core/Grid';
import Typography from '@material-ui/core/Typography';
import { withRouter } from "react-router-dom";
import defaultProfileImage from '../../images/no-img.png'

const styles = (theme) => ({
  ...theme,
  commentImage: {
    maxWidth: '100%',
    height: 100,
    objectFit: 'cover',
    borderRadius: '50%'
  },
  commentData: {
    marginLeft: 20
  }
});

class Comments extends Component {
  constructor() {
    super()
    this.onClickUserHandle = this.onClickUserHandle.bind(this);
  }
  
  onClickUserHandle(e) {
    this.props.history.push(`/users/${e.target.name}`);
  }

  render() {
    const { comments, classes } = this.props;
    return (
      <Grid container>
        {comments.map((comment, index) => {
          const { body, createdAt, userImage, userHandle } = comment;
          return (
            <Fragment key={createdAt}>
              <Grid item sm={12}>
                <Grid container>
                  <Grid item sm={1}>
                    <img
                      src={(userImage === '' || !userImage) ? defaultProfileImage : userImage}
                      alt="comment"
                      className={classes.commentImage}
                    />
                  </Grid>
                  <Grid item sm={11}>
                    <div className={classes.commentData}>
                      <Typography
                        variant="h5"
                        color="primary"
                      >
                        <div onClick={this.onClickUserHandle} name={userHandle}>{userHandle}</div>
                      </Typography>
                      <Typography variant="body2" color="textSecondary">
                        {dayjs(createdAt).format('h:mm a, MMMM DD YYYY')}
                      </Typography>
                      <hr className={classes.invisibleSeparator} />
                      <div dangerouslySetInnerHTML={{ __html: " <Typography variant=\"body1\">" + body + "</Typography>"}}/>
                    </div>
                  </Grid>
                </Grid>
              </Grid>
              {index !== comments.length - 1 && (
                <hr className={classes.visibleSeparator} />
              )}
            </Fragment>
          );
        })}
      </Grid>
    );
  }
}

Comments.propTypes = {
  comments: PropTypes.array.isRequired
};

export default withStyles(styles)(withRouter(Comments));
